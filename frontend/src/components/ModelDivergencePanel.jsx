...
export default function ModelDivergencePanel({ modelSynthesis }) {
  ...
  const severityColors = {
    high: 'bg-civic-gray-600/20 text-civic-gray-300 border-civic-gray-600/30',
    medium: 'bg-civic-gray-500/20 text-civic-gray-300 border-civic-gray-500/30',
    low: 'bg-civic-gray-400/20 text-civic-gray-300 border-civic-gray-400/30',
  };
      ...
      {modelSynthesis.divergences?.hasDivergences && (
        <div className="bg-civic-dark-900/50 rounded-lg border border-civic-dark-700 p-4">
          <div className="flex items-center gap-2 mb-3">
            <svg className="w-5 h-5 text-civic-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="bg-civic-dark-900/50 rounded-lg border border-civic-dark-700 p-4">
          ...
          <div className="space-y-2">
            {modelSynthesis.contradictions.contradictions?.map((contr, idx) => (
              <div key={idx} className="rounded-lg border border-civic-gray-600/30 bg-civic-gray-600/10 p-3">
                <div className="flex items-start gap-2">
                  <span className="text-base text-civic-gray-400">●</span>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs font-medium text-civic-gray-300">{contr.topic}</span>
                      <span className="text-[10px] px-1.5 py-0.5 bg-black/20 rounded text-civic-gray-400 capitalize">
                        {contr.type}
                      </span>
                    </div>
                    <p className="text-xs text-civic-gray-400 mb-2">{contr.description}</p>
                    ...
                    {contr.models && (
                      <div className="flex flex-wrap gap-1.5">
                        {contr.models.map((model, midx) => (
                          <span 
                            key={midx}
                            className="text-[10px] px-2 py-1 bg-black/20 rounded text-civic-gray-400"
                          >
